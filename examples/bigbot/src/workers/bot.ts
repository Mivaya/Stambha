import { HttpRestPort, gatesDesiredProperties, minimalDesiredProperties } from "@stambha/core";
...
const desired =
  mode === "gates" ? gatesDesiredProperties : mode === "minimal" ? minimalDesiredProperties : undefined;
...
const server = await createWorkerServer({
  ...
  onMessage: async (message) => {
    if (message.type === WorkerMessageTypes.gatewayReady) {
      ...
      console.log("[bigbot split:bot] ready via gateway relay");
      return;
    }
...
console.log(`Bigbot worker at ${server.url} (REST via ${restUrl})`);
