...
import type { DesiredProperties, RestPort, RestRequest, StambhaClient } from "@stambha/core";
...
export interface BotSetupOptions {
  ...
  demo?: boolean;
  /** Slim context fields for large bots — see Desired Properties docs. */
...
export async function setupBot(options: BotSetupOptions = {}): Promise<BotSetupResult> {
  ...
  const client = createStambhaBot({
    ...
    ...(options.desiredProperties ? { desiredProperties: options.desiredProperties } : {}),
  });
