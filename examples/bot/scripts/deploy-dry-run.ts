/**
 * CI / local dry-run — validates slash payload from example-bot pieces (no Discord token).
 */
...
if (result.count < 1) {
  console.error("[deploy:dry-run] expected at least one slash command in example-bot");
  process.exit(1);
}
